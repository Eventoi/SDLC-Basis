...
> Например, ПО не инсталлируется (блокирует использование ПО); в интернет-магазине нельзя купить товар (бессмысленно использовать такой интернет-магазин); ошибка соединения с БД (невозможно управлять данными) и т.д.

> [!TIP]
> Чтобы собрать такой набор, необходимо выделить суть тестируемого ПО и проверять те модули, которые её реализуют
